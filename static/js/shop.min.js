    ...
    t.extend(i.prototype,
        {
            init:function(){
                ...
                if(a.cart=a.cart.append("<ol/>").find("ol"),
                    ...
                    "true"===a.settings.display_total_value&&"false"===a.settings.permanent_total_value&&a.append_total(),
                        ...
                        a.quantity_change(i),
                        a.remove_item(r)}a.settings.init&&a.settings.init(),
                    a.cart_empty(),
                    a.product.each(function(e){
                        ...
                        s.on("click touchend",
                            ...
                            d.val();
                            return 0!==l.length?p!==u?a.add_to_cart(c,o,u):_.find("input").val(function(t,e){return parseInt(e,10)+1}):(
                                    a.add_to_cart(c,o,u),
                                    "false"===a.settings.permanent_cart_buttons&&a.add_cart_buttons(),
                                    "true"===a.settings.display_total_value&&"false"===a.settings.permanent_total_value&&a.append_total()
                            ),
            ...
            add_to_cart:function(t,e,a){
                var n=this;
                n.cart.append(
                    "true"===n.settings.currency_after_number?
                        '<li class="animated ' +n.settings.animation +'" data-id='+a+">" +
                        "<span class="+n.settings.cart+"-name>"+t+"</span>" +
                        "<span class="+n.settings.cart+"-price>"
                        +e+n.settings.currency+
                        '</span>' +
                        '<input type="number" min="1" value="1" class='+n.settings.cart+"-input>" +
                        "<button class="+n.settings.cart+"-remove>&times;</button>" +
                        "</li>":
                        '<li class="animated '+n.settings.animation+'" data-id='+a+">" +
                        "<span class="+n.settings.cart+"-name>"+t+"</span>" +
                        "<span class="+n.settings.cart+"-price>"+n.settings.currency+e+'</span>' +
                        '<input type="number" min="1" value="1" class='+n.settings.cart+"-input>" +
                        "<button class="+n.settings.cart+"-remove>&times;</button>" +
                        "</li>"),
                    n.storage_set()
            ...
            add_cart_buttons:function(){
                var t=this;
                t.cart.parent().append('<button class="'+t.settings.cart+'-clear">清空</button>'),
                    t.cart.parent().append('<button type="submit" class="'+t.settings.cart+'-checkout">结算</button>'),
                    t.clear_cart(),
                    t.checkout_button()},
            checkout:function(){
                var e,a=this,
                ...
                for(var s in n)e=t('<input type="hidden" />'),
                    e.attr("name",s),e.attr("value",n[s]),
                    i.append(e);
            ...
            checkout_button:function(){
                var e=this;
                t("."+e.settings.cart+"-checkout").unbind().on(
                    "click",
                    function(){
                        // e.settings.before_checkout&&e.settings.before_checkout()
                        //     ,e.checkout();
                        e.storage_set();
                        var totle_value = e.calculate_total_value();
                        var dic = sessionStorage.getItem('cart');
                        $.ajax({
                            url:'/balance/',
                            type:'post',
                            data:{
                                totle_value:totle_value,
                                dic:dic,
                                csrfmiddlewaretoken:$
                            ...
                            success:function(data){
                                if(data.isOk == 1){
                                    var d = data.text;
                                    var r = confirm(d); //让用户确认订单
                                    if(r==true){
                                        window.open("/pay_for/");
                                        e.cart.find("li").each(function(){t(this).remove()}),
            ...
            remove_item:function(e){var a=this;
            	t("."+a.settings.cart+"-remove").unbind().on("click",function(){
            		var e=t(this),
            		n=a.cart.find("li").length;e.parent().remove(),
            ...
            remove_cart_buttons:function(){
            	var t=this;"false"===t.settings.permanent_cart_buttons&&t.cart.parent().find("button").remove()
            },
            check_if_title:function(t,e){
            	return 0!==t.find("select").length?e=0!==t.find("span").length?t.find("span").text()+" - "+t.find("option:selected").val():t.find("option:selected").text():
            	0!==t.find("input").length?e=t.find("input").val():t.is(":empty")&&(e=t.val()),e
            },
            ...
            permanent_cart_buttons:function(){
            	var t=this;"true"===t.settings.permanent_cart_buttons&&t.add_cart_buttons()
            },
            ...
            storage_get:function(){
            		var t=e.sessionStorage.cart;return t=JSON.parse(t)
            	},
            ...
            create_storage_cart:function(){
            		for(var t=this,e=t.storage_get(),
            			...
            			t.cart.append('<li class="animated '+t.settings.animation+'" data-id='+i+
            				"><span class="+t.settings.cart+"-name>"+r+
            				"</span><span class="+t.settings.cart+"-price>"+s+
            				'</span><input type="number" min="1" value="'+c+'" class='+
            				t.settings.cart+"-input><button class="+t.settings.cart+"-remove>x</button></li>")};
            		},
            cart_empty:function(){
            			var t=this,
            			e=t.settings.cart+"-empty";
            			"false"===t.settings.empty_disable&&(t.cart.find("li").length?$("."+e).remove():t.cart.parent().prepend(
            				'<span class="'+e+'">'+'暂未加入商品'+"</span>"))}
        }),
t.fn[r]=function(e){
	return this.each(function(){
		t.data(this,"plugin_"+r)||t.data(this,"plugin_"+r,new i(this,e))})}}(jQuery,window,document);
